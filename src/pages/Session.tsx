...
import { ArrowLeft, Play, Edit, Clock, Flame, Loader2, Activity } from "lucide-react";
  ...
  useEffect(() => {
    async function loadWorkout() {
      if (state?.mode === "today_auto") {
        ...
        // Generate workout via LLM (adjusted flow)
        setIsGenerating(true);
        const userEquipment = await loadEquipment();
        ...
        try {
          ...
          toast({
            title: "Using Quick Workout",
            description:
              "We couldn't generate a custom workout right now, so we've created a quick one for you. You can try refining it again.",
            variant: "default",
          });
    ...
    loadWorkout();
  }, [state, navigate, toast, user?.id, session?.access_token]);

  const handleStartWorkout = () => {
    if (!workout) return;

    // Build workout session and save to localStorage
    const workoutSession = buildWorkoutSession(workout);
    saveWorkoutSession(workoutSession);

    // Navigate to first step of workout player
    navigate(`/workout/${workoutSession.id}/0`);
  };
      ...
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center space-y-4">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-2">Personalizing exercises based on your energy and goals</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-background pb-24">
      ...
      <header className="bg-primary text-primary-foreground px-6 pt-6 pb-6 rounded-b-3xl shadow-lg sticky top-0 z-10">
        <div className="max-w-md mx-auto">
          ...
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl font-bold text-primary-foreground">Your Workout</h1>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-md mx-auto px-6 py-6 space-y-6">
        ...
        {/* Exercise List */}
        <section>
          <h3 className="text-lg font-semibold text-foreground mb-3">Exercises ({workout.exercises.length})</h3>
        ...
        <div className="space-y-3 pt-4">
          ...
          <Button variant="outline" size="lg" className="w-full" onClick={handleRefine}>
